...
const validate = (values) => {
  ...
  if (values.hasOwnProperty('username') && values.hasOwnProperty('tel')) {
    ...
    if (!values.tel) {
      errors.tel = 'Required';
    } else if (!isPossiblePhoneNumber(values.tel.toString())) {
      errors.tel = 'Too long to be a valid phone number';
    } else if (!isValidPhoneNumber(values.tel.toString())) {
      errors.tel = 'Invalid phone numer';
    }
  }
  // email and password
  if (!values.email) {
        ...
        {props.formType === 'signup' ? (
          <Fragment>
            ...
            <PhoneInput
              ...
              onBlur={(e) => formik.setFieldTouched('tel', e)}
